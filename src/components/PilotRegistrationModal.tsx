        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="Ex: Nome Completo do Piloto"
                required
                autoFocus
                className="w-full bg-white border-2 border-black rounded-lg pl-9 pr-3 py-2 text-sm font-bold text-black placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-900"
              />
            </div>

            {/* Dynamic Backup Match Feedback */}
            {isExactBackupFound && backupSummary && (
              <div className="mt-2 p-2 bg-emerald-50 border border-emerald-500 rounded-lg text-xs text-emerald-950 flex items-center gap-2">
                <Check className="w-3.5 h-3.5 text-emerald-600 shrink-0 stroke-[3]" />
                <div className="leading-tight">
                  <strong className="text-emerald-900">Piloto Reconhecido:</strong> Cópia de segurança com {backupSummary.maneuverCount} manobras restaurada.
                </div>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full bg-white border-2 border-black rounded-lg pl-9 pr-3 py-2 text-sm font-bold text-black focus:outline-none focus:ring-2 focus:ring-blue-900"
                required
              >
                ...
                <option value="Piloto Chefe / Coordenador">Piloto Chefe / Coordenador</option>
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <div className="relative">
                <FileBadge className="w-4 h-4 text-slate-500 absolute left-3 top-2.5" />
                <input
                  type="text"
                  value={licenseNumber}
                  onChange={(e) => setLicenseNumber(e.target.value)}
                  placeholder="Ex: CIR-84920 ou Nº Licença"
                  className="w-full bg-white border-2 border-black rounded-lg pl-9 pr-3 py-2 text-sm font-semibold text-black placeholder:text-slate-400"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <Radio className="w-4 h-4 text-slate-500 absolute left-3 top-2.5" />
                <input
                  type="text"
                  value={vhfCallSign}
          ...
          {/* Action Buttons */}
          <div className="pt-3 border-t-2 border-black flex flex-col sm:flex-row items-center justify-between gap-2.5">
            {!isMandatory ? (
              <button
                type="button"
                onClick={handleLogout}
                className="w-full sm:w-auto px-3 py-1.5 text-rose-700 hover:bg-rose-50 border border-rose-300 rounded-md font-bold text-xs flex items-center justify-center gap-1.5 transition-colors"
              >
            ...
            <div className="w-full sm:w-auto flex items-center gap-2">
              {!isMandatory && (
                <button
                  type="button"
                  onClick={() => setIsProfileModalOpen(false)}
                  className="w-full sm:w-auto px-3 py-1.5 rounded-md border-2 border-black font-bold text-xs text-black hover:bg-slate-100"
                >
                  Cancelar
                </button>
              )}
              ...
              <button
                type="submit"
                className="w-full sm:w-auto px-4 py-1.5 rounded-md border-2 border-black bg-blue-900 hover:bg-blue-800 text-white font-black text-xs tracking-wide shadow-xs flex items-center justify-center gap-1.5 active:scale-95 transition-all"
              >
                <Check className="w-3.5 h-3.5 stroke-[3] text-cyan-300" />
                <span>{currentUser ? 'Guardar' : 'Confirmar'}</span>
              </button>
            </div>
          </div>
        </form>
